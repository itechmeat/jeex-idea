...
# Configuration
POSTGRES_CONTAINER="jeex-postgres"
...
# Execute PostgreSQL command (uses Unix socket with trust authentication)
exec_psql() {
    local query="$1"
    # Validate input to prevent SQL injection
    if [[ ! "$query" =~ ^[a-zA-Z0-9_[:space:],\(\)'\"-]+$ ]]; then
        log_error "Invalid characters in SQL query"
        return 1
    fi
...
test_uuid_v7() {
    ...
    if exec_psql "SELECT gen_random_uuid();" >/dev/null 2>&1; then
        log_success "UUID v7 (gen_random_uuid) function available"
    else
        log_warning "UUID v7 not available, falling back to uuid-ossp"

        # Test uuid-ossp fallback
        if exec_psql "SELECT uuid_generate_v4();" >/dev/null 2>&1; then
            log_success "UUID generation (uuid-ossp) available as fallback"
        else
...
test_database_schema() {
    ...
    if exec_psql "\\dt" | grep -q "app_config"; then
    ...
    if exec_psql "\\df" | grep -q "check_database_health"; then
...
test_user_accounts() {
    ...
    # Check jeex_user
    if exec_psql "\\du" | grep -q "$POSTGRES_USER"; then
    ...
    # Check jeex_admin
    if exec_psql "\\du" | grep -q "jeex_admin"; then
    ...
    # Check jeex_readonly
    if exec_psql "\\du" | grep -q "jeex_readonly"; then
...
test_host_connection() {
    ...
    if PGPASSWORD="jeex_secure_password" psql -h localhost -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT 1;" >/dev/null 2>&1; then
        log_success "Host connection to PostgreSQL successful"
    else
        log_warning "Host connection to PostgreSQL failed (container might not expose port)"
    fi
}
